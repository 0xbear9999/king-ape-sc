...
declare_id!("Q1Y4tckggicbdc7pNWzzcAkfr8gBGfjQcqnyCNNmTnt");
...
pub mod king_kong_game {
    ...
    pub fn init_user_pool(ctx: Context<InitUserPool>, _bump: u8) -> Result<()> {
        msg!("Init User Pool for 16th player");
    ...
    pub fn buy_token(
        ...
        let seeds = &[ESCROW_VAULT_SEED.as_bytes(), &[_escrow_bump]];
        let signer = &[&seeds[..]];
        // transfer sol to banana_treasury
        invoke(
            &system_instruction::transfer(
                ctx.accounts.buyer.key,
                ctx.accounts.banana_treasury.key,
                game_config_vault.banana_price * token_amount / game_config_vault.banana_decimal,
            ),
            &[
                ctx.accounts.buyer.to_account_info().clone(),
                ctx.accounts.banana_treasury.to_account_info().clone(),
                ctx.accounts.system_program.to_account_info().clone(),
            ],
        )?;
...
pub struct PlayGame<'info> {
    ...
    pub winner_pda: Box<Account<'info, UserPool>>,
...
pub struct BuyToken<'info> {
    ...
    #[account(
        mut,
        constraint = banana_treasury.key() == BANANA_TREASURY_WALLET.parse::<Pubkey>().unwrap(),
    )]
    /// CHECK: This is not dangerous because we don't read or write from this account
    pub banana_treasury: AccountInfo<'info>,
